...
> **타임존 주의**: 폼 응답 시트의 타임존이 `Etc/GMT` (UTC)로 설정되어 있다. 대시보드 시트는 `Asia/Seoul` (KST). IMPORTRANGE는 serial number를 그대로 전달하므로, 수식에서 `+9/24`로 KST 보정이 필요하다.
...
for row in range(2, 8):  # 2행 ~ 7행
    cell_ref = f"A{row}"
    # 주차별 제출 수식 (C~E열)
    # SUMPRODUCT로 4가지 조건 집계: 이름 존재 × 팀 일치 × 주차 일치 × 해당 주차 마감일 이전
    # +9/24: 원본 폼 시트가 Etc/GMT(UTC)이므로 KST 변환 보정
    week_formula_template = (
        "=IF(COUNTA('과제 폼 가져오기'!$C$2:$C)=0, 0, "
        "SUMPRODUCT("
        "('과제 폼 가져오기'!$C$2:$C<>\"\")"
        "*('과제 폼 가져오기'!$E$2:$E=$A{row})"
        "*ISNUMBER(SEARCH((COLUMN()-2)&\"주차\", '과제 폼 가져오기'!$F$2:$F))"
        "*('과제 폼 가져오기'!$A$2:$A+9/24<=$B$10+(COLUMN()-3)*7)))"
    )
    formulas.append([
        # B열: 팀원수
        f"=COUNTIFS('소그룹 데이터 가져오기'!B:B, {cell_ref})",
        # C열: 1주차 제출 (COLUMN()-2=1)
        week_formula_template.format(row=row),
        # D열: 2주차 제출 (COLUMN()-2=2)
        week_formula_template.format(row=row),
        # E열: 3주차 제출 (COLUMN()-2=3)
        week_formula_template.format(row=row),
        # F열: 누적 개인점수 (15인 정규화)
        f"=SUM(C{row}:E{row}) * 10 * (15 / B{row})",
        # G열: 누적 보너스
        f'=COUNTIF(C{row}:E{row}, ">="&B{row}) * 100',
        # H열: 총점
        f"=F{row} + G{row}",
    ])
...
| 열 | 수식 | 설명 |
|----|------|------|
| B (팀원수) | `=COUNTIFS('소그룹 데이터 가져오기'!B:B, A2)` | 소그룹 시트에서 해당 팀에 배정된 인원 수 |
| C~E (주차별 제출) | `=IF(COUNTA(...)=0, 0, SUMPRODUCT(...))` | 이름 존재 × 팀 일치 × 주차 일치 × 해당 주차 마감일 이전 → 4가지 조건으로 집계 |
| F (누적 개인) | `=SUM(C2:E2) * 10 * (15 / B2)` | (총 제출 건수) x 10점 x (15 / 팀원수) -- 15인 기준 정규화 |
| G (누적 보너스) | `=COUNTIF(C2:E2, ">="&B2) * 100` | 전원 제출 달성 주차 수 x 100점 |
| H (총점) | `=F2 + G2` | 개인점수 + 보너스 합산 |
...
=IF(COUNTA('과제 폼 가져오기'!$C$2:$C)=0, 0,
  SUMPRODUCT(
    ('과제 폼 가져오기'!$C$2:$C<>"")           ← 이름 비어있지 않음
    *('과제 폼 가져오기'!$E$2:$E=$A2)          ← 팀 일치
    *ISNUMBER(SEARCH((COLUMN()-2)&"주차",
      '과제 폼 가져오기'!$F$2:$F))             ← 주차 일치 (COLUMN()-2로 자동 변환)
    *('과제 폼 가져오기'!$A$2:$A+9/24<=$B$10+(COLUMN()-3)*7) ← 해당 주차 마감일 이전 제출만 인정
  ))
```

| 구성 요소 | 설명 |
|-----------|------|
| `SUMPRODUCT` | 시간 함수 조건을 지원하기 위해 COUNTIFS 대신 사용 |
| `+9/24` | 원본 폼 시트 타임존이 UTC이므로 KST(+9시간)로 변환 |
| `<=$B$10+(COLUMN()-3)*7` | 해당 주차 마감일 이전 제출만 인정 ($B$10=1주차 마감일, 주차마다 7일 증가) |
| `COLUMN()-2` | C=1주차, D=2주차, E=3주차 자동 대응 |
...
점수 집계 시트의 **A10:B12** 영역에 주차별 마감일이 설정되어 있다.

| 셀 | 내용 | 예시 |
|----|------|------|
| A10/B10 | 1주차 과제 마감일 | `2026-02-18 23:00:00` |
| A11/B11 | 2주차 과제 마감일 | `=B10+7` (자동 계산) |
| A12/B12 | 3주차 과제 마감일 | `=B10+14` (자동 계산) |

**기수가 바뀔 때 B10 셀 하나만 변경하면 된다.**

### 피드백요청 보너스

점수 집계 시트의 **I~L열**에 피드백요청 보너스가 설정되어 있다.

| 열 | 헤더 | 입력 방식 |
|----|------|-----------|
| I~K | 피드백 1~3주차 | 태규님이 0(미완료) 또는 1(전원완료) 직접 입력 |
| L | 피드백 보너스 | `=COUNTIF(I:K,1)*100` 자동 계산 |

팀 전원이 '#피드백요청'을 톡방에 남기면 해당 주차에 100점 보너스. 총점(H열) = 개인점수(F) + 과제보너스(G) + 피드백보너스(L).

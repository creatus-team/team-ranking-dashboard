...
for row in range(2, 8):  # 2행 ~ 7행
    cell_ref = f"A{row}"
    # 주차별 제출 수식 (C~E열)
    # SUMPRODUCT로 4가지 조건 집계: 이름 존재 × 팀 일치 × 주차 일치 × KST 23시 이전
...
| C~E (주차별 제출) | `=IF(COUNTA(...)=0, 0, SUMPRODUCT(...))` | 이름 존재 × 팀 일치 × 주차 일치 × KST 23시 이전 → 4가지 조건으로 집계 |
...
=IF(COUNTA('과제 폼 가져오기'!$C$2:$C)=0, 0,
  SUMPRODUCT(
    ...
    *(HOUR('과제 폼 가져오기'!$A$2:$A+9/24)<23) ← KST 23시 이전 제출만 인정
  ))
```
...
| `HOUR(...)<23` | KST 기준 오후 11시(23시) 이후 제출은 제외 |
